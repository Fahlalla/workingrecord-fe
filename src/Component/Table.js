const Table = () => {
  return (
    <table className="space-y-4 text-center ">
      <thead>
        <tr>
          <th className="w-2/12">Name</th>
          <th className="w-2/12">Email</th>
          <th className="w-2/12">Phone Number</th>
          <th className="w-2/12">Contact</th>
          <th className="w-2/12">Site</th>
          <th className="w-2/12">Status</th>
          <th className="w-2/12">Bell</th>
        </tr>
      </thead>
      <div className="hover:bg-black-200 w-full"></div>
      <tbody>
        <tr>
          <td>Tadsika Khongkasawan</td>
          <td>Tadsika@odds.team</td>
          <td>0818888888</td>
          <td>Jill(0888888888)</td>
          <td>odds</td>
          <td>
            <button className="text-green-600 border-2 border-green-600 rounded-3xl w-28">
              Submitted
            </button>
          </td>
          <td>
            <svg
              xmlns="http://www.w3.org/2000/svg"
              height="24px"
              viewBox="0 0 24 24"
              width="24px"
              fill="#444444"
            >
              <path d="M12 22c1.1 0 2-.9 2-2h-4c0 1.1.89 2 2 2zm6-6v-5c0-3.07-1.64-5.64-4.5-6.32V4c0-.83-.67-1.5-1.5-1.5s-1.5.67-1.5 1.5v.68C7.63 5.36 6 7.92 6 11v5l-2 2v1h16v-1l-2-2z" />
            </svg>
          </td>
        </tr>
        <tr>
          <td>Tadsika Khongkasawan</td>
          <td>Tadsika@odds.team</td>
          <td>0818888888</td>
          <td>Jill(0888888888)</td>
          <td>odds</td>
          <td>
            <button className="text-green-600 border-2 border-green-600 rounded-3xl w-28">
              Submitted
            </button>
          </td>
          <td>
            <svg
              xmlns="http://www.w3.org/2000/svg"
              height="24px"
              viewBox="0 0 24 24"
              width="24px"
              fill="#444444"
            >
              <path d="M12 22c1.1 0 2-.9 2-2h-4c0 1.1.89 2 2 2zm6-6v-5c0-3.07-1.64-5.64-4.5-6.32V4c0-.83-.67-1.5-1.5-1.5s-1.5.67-1.5 1.5v.68C7.63 5.36 6 7.92 6 11v5l-2 2v1h16v-1l-2-2z" />
            </svg>
          </td>
        </tr>
        <tr>
          <td>Tadsika Khongkasawan</td>
          <td>Tadsika@odds.team</td>
          <td>0818888888</td>
          <td>Jill(0888888888)</td>
          <td>odds</td>
          <td>
            <button className="text-red-600 border-2 border-red-600 rounded-3xl w-28">
              Submitted
            </button>
          </td>
          <td>
            <svg
              xmlns="http://www.w3.org/2000/svg"
              height="24px"
              viewBox="0 0 24 24"
              width="24px"
              fill="#444444"
            >
              <path d="M12 22c1.1 0 2-.9 2-2h-4c0 1.1.89 2 2 2zm6-6v-5c0-3.07-1.64-5.64-4.5-6.32V4c0-.83-.67-1.5-1.5-1.5s-1.5.67-1.5 1.5v.68C7.63 5.36 6 7.92 6 11v5l-2 2v1h16v-1l-2-2z" />
            </svg>
          </td>
        </tr>
        <tr>
          <td>Tadsika Khongkasawan</td>
          <td>Tadsika@odds.team</td>
          <td>0818888888</td>
          <td>Jill(0888888888)</td>
          <td>odds</td>
          <td>
            <button className="text-red-600 border-2 border-red-600 rounded-3xl w-28">
              Submitted
            </button>
          </td>
          <td>
            <svg
              xmlns="http://www.w3.org/2000/svg"
              height="24px"
              viewBox="0 0 24 24"
              width="24px"
              fill="#444444"
            >
              <path d="M12 22c1.1 0 2-.9 2-2h-4c0 1.1.89 2 2 2zm6-6v-5c0-3.07-1.64-5.64-4.5-6.32V4c0-.83-.67-1.5-1.5-1.5s-1.5.67-1.5 1.5v.68C7.63 5.36 6 7.92 6 11v5l-2 2v1h16v-1l-2-2z" />
            </svg>
          </td>
        </tr>
        <tr>
          <td>pui pui</td>
          <td>pui@odds.Team</td>
          <td>0818888888</td>
          <td>Jill(0888888888)</td>
          <td>odds</td>
          <td>
            <button className="text-red-600 border-2 border-red-600 rounded-3xl w-28">
              Submitted
            </button>
          </td>
          <td>
            <svg
              xmlns="http://www.w3.org/2000/svg"
              height="24px"
              viewBox="0 0 24 24"
              width="24px"
              fill="#444444"
            >
              <path d="M12 22c1.1 0 2-.9 2-2h-4c0 1.1.89 2 2 2zm6-6v-5c0-3.07-1.64-5.64-4.5-6.32V4c0-.83-.67-1.5-1.5-1.5s-1.5.67-1.5 1.5v.68C7.63 5.36 6 7.92 6 11v5l-2 2v1h16v-1l-2-2z" />
            </svg>
          </td>
        </tr>
        <tr>
          <td>pui pui</td>
          <td>pui@odds.Team</td>
          <td>0818888888</td>
          <td>Jill(0888888888)</td>
          <td>odds</td>
          <td>
            <button className="text-red-600 border-2 border-red-600 rounded-3xl w-28">
              Submitted
            </button>
          </td>
          <td>
            <svg
              xmlns="http://www.w3.org/2000/svg"
              height="24px"
              viewBox="0 0 24 24"
              width="24px"
              fill="#444444"
            >
              <path d="M12 22c1.1 0 2-.9 2-2h-4c0 1.1.89 2 2 2zm6-6v-5c0-3.07-1.64-5.64-4.5-6.32V4c0-.83-.67-1.5-1.5-1.5s-1.5.67-1.5 1.5v.68C7.63 5.36 6 7.92 6 11v5l-2 2v1h16v-1l-2-2z" />
            </svg>
          </td>
        </tr>
        <tr>
          <td>pui pui</td>
          <td>pui@odds.Team</td>
          <td>0818888888</td>
          <td>Jill(0888888888)</td>
          <td>odds</td>
          <td>
            <button className="text-red-600 border-2 border-red-600 rounded-3xl w-28">
              Submitted
            </button>
          </td>
          <td>
            <svg
              xmlns="http://www.w3.org/2000/svg"
              height="24px"
              viewBox="0 0 24 24"
              width="24px"
              fill="#444444"
            >
              <path d="M12 22c1.1 0 2-.9 2-2h-4c0 1.1.89 2 2 2zm6-6v-5c0-3.07-1.64-5.64-4.5-6.32V4c0-.83-.67-1.5-1.5-1.5s-1.5.67-1.5 1.5v.68C7.63 5.36 6 7.92 6 11v5l-2 2v1h16v-1l-2-2z" />
            </svg>
          </td>
        </tr>
        <tr>
          <td>pui pui</td>
          <td>pui@odds.Team</td>
          <td>0818888888</td>
          <td>Jill(0888888888)</td>
          <td>odds</td>
          <td>
            <button className="text-red-600 border-2 border-red-600 rounded-3xl w-28">
              Submitted
            </button>
          </td>
          <td>
            <svg
              xmlns="http://www.w3.org/2000/svg"
              height="24px"
              viewBox="0 0 24 24"
              width="24px"
              fill="#444444"
            >
              <path d="M12 22c1.1 0 2-.9 2-2h-4c0 1.1.89 2 2 2zm6-6v-5c0-3.07-1.64-5.64-4.5-6.32V4c0-.83-.67-1.5-1.5-1.5s-1.5.67-1.5 1.5v.68C7.63 5.36 6 7.92 6 11v5l-2 2v1h16v-1l-2-2z" />
            </svg>
          </td>
        </tr>
        <tr>
          <td>pui pui</td>
          <td>pui@odds.Team</td>
          <td>0818888888</td>
          <td>Jill(0888888888)</td>
          <td>odds</td>
          <td>
            <button className="text-red-600 border-2 border-red-600 rounded-3xl w-28">
              Submitted
            </button>
          </td>
          <td>
            <svg
              xmlns="http://www.w3.org/2000/svg"
              height="24px"
              viewBox="0 0 24 24"
              width="24px"
              fill="#444444"
            >
              <path d="M12 22c1.1 0 2-.9 2-2h-4c0 1.1.89 2 2 2zm6-6v-5c0-3.07-1.64-5.64-4.5-6.32V4c0-.83-.67-1.5-1.5-1.5s-1.5.67-1.5 1.5v.68C7.63 5.36 6 7.92 6 11v5l-2 2v1h16v-1l-2-2z" />
            </svg>
          </td>
        </tr>
        <tr>
          <td>pui pui</td>
          <td>pui@odds.Team</td>
          <td>0818888888</td>
          <td>Jill(0888888888)</td>
          <td>odds</td>
          <td>
            <button className="text-red-600 border-2 border-red-600 rounded-3xl w-28">
              Submitted
            </button>
          </td>
          <td>
            <svg
              xmlns="http://www.w3.org/2000/svg"
              height="24px"
              viewBox="0 0 24 24"
              width="24px"
              fill="#444444"
            >
              <path d="M12 22c1.1 0 2-.9 2-2h-4c0 1.1.89 2 2 2zm6-6v-5c0-3.07-1.64-5.64-4.5-6.32V4c0-.83-.67-1.5-1.5-1.5s-1.5.67-1.5 1.5v.68C7.63 5.36 6 7.92 6 11v5l-2 2v1h16v-1l-2-2z" />
            </svg>
          </td>
        </tr>
        <tr>
          <td>pui pui</td>
          <td>pui@odds.Team</td>
          <td>0818888888</td>
          <td>Jill(0888888888)</td>
          <td>odds</td>
          <td>
            <button className="text-red-600 border-2 border-red-600 rounded-3xl w-28">
              Submitted
            </button>
          </td>
          <td>
            <svg
              xmlns="http://www.w3.org/2000/svg"
              height="24px"
              viewBox="0 0 24 24"
              width="24px"
              fill="#444444"
            >
              <path d="M12 22c1.1 0 2-.9 2-2h-4c0 1.1.89 2 2 2zm6-6v-5c0-3.07-1.64-5.64-4.5-6.32V4c0-.83-.67-1.5-1.5-1.5s-1.5.67-1.5 1.5v.68C7.63 5.36 6 7.92 6 11v5l-2 2v1h16v-1l-2-2z" />
            </svg>
          </td>
        </tr>

        <tr>
          <td>pui pui</td>
          <td>pui@odds.Team</td>
          <td>0818888888</td>
          <td>Jill(0888888888)</td>
          <td>odds</td>
          <td>
            <button className="text-red-600 border-2 border-red-600 rounded-3xl w-28">
              Submitted
            </button>
          </td>
          <td>
            <svg
              xmlns="http://www.w3.org/2000/svg"
              height="24px"
              viewBox="0 0 24 24"
              width="24px"
              fill="#444444"
            >
              <path d="M12 22c1.1 0 2-.9 2-2h-4c0 1.1.89 2 2 2zm6-6v-5c0-3.07-1.64-5.64-4.5-6.32V4c0-.83-.67-1.5-1.5-1.5s-1.5.67-1.5 1.5v.68C7.63 5.36 6 7.92 6 11v5l-2 2v1h16v-1l-2-2z" />
            </svg>
          </td>
        </tr>
        <tr>
          <td>pui pui</td>
          <td>pui@odds.Team</td>
          <td>0818888888</td>
          <td>Jill(0888888888)</td>
          <td>odds</td>
          <td>
            <button className="text-red-600 border-2 border-red-600 rounded-3xl w-28">
              Submitted
            </button>
          </td>
          <td>
            <svg
              xmlns="http://www.w3.org/2000/svg"
              height="24px"
              viewBox="0 0 24 24"
              width="24px"
              fill="#444444"
            >
              <path d="M12 22c1.1 0 2-.9 2-2h-4c0 1.1.89 2 2 2zm6-6v-5c0-3.07-1.64-5.64-4.5-6.32V4c0-.83-.67-1.5-1.5-1.5s-1.5.67-1.5 1.5v.68C7.63 5.36 6 7.92 6 11v5l-2 2v1h16v-1l-2-2z" />
            </svg>
          </td>
        </tr>
        <tr>
          <td>pui pui</td>
          <td>pui@odds.Team</td>
          <td>0818888888</td>
          <td>Jill(0888888888)</td>
          <td>odds</td>
          <td>
            <button className="text-red-600 border-2 border-red-600 rounded-3xl w-28">
              Submitted
            </button>
          </td>
          <td>
            <svg
              xmlns="http://www.w3.org/2000/svg"
              height="24px"
              viewBox="0 0 24 24"
              width="24px"
              fill="#444444"
            >
              <path d="M12 22c1.1 0 2-.9 2-2h-4c0 1.1.89 2 2 2zm6-6v-5c0-3.07-1.64-5.64-4.5-6.32V4c0-.83-.67-1.5-1.5-1.5s-1.5.67-1.5 1.5v.68C7.63 5.36 6 7.92 6 11v5l-2 2v1h16v-1l-2-2z" />
            </svg>
          </td>
        </tr>

        <tr>
          <td>pui pui</td>
          <td>pui@odds.Team</td>
          <td>0818888888</td>
          <td>Jill(0888888888)</td>
          <td>odds</td>
          <td>
            <button className="text-red-600 border-2 border-red-600 rounded-3xl w-28">
              Submitted
            </button>
          </td>
          <td>
            <svg
              xmlns="http://www.w3.org/2000/svg"
              height="24px"
              viewBox="0 0 24 24"
              width="24px"
              fill="#444444"
            >
              <path d="M12 22c1.1 0 2-.9 2-2h-4c0 1.1.89 2 2 2zm6-6v-5c0-3.07-1.64-5.64-4.5-6.32V4c0-.83-.67-1.5-1.5-1.5s-1.5.67-1.5 1.5v.68C7.63 5.36 6 7.92 6 11v5l-2 2v1h16v-1l-2-2z" />
            </svg>
          </td>
        </tr>
      </tbody>
    </table>
  );
};
export default Table;
